...
async def main():
    ...
    with open('Marketing_details.csv', 'a', newline='', encoding='utf-8') as csvfile:
        ...
        async with aiohttp.ClientSession() as session:
            for i, search_url in enumerate(url_list[start_index:], start=start_index):
                url_counter += 1
                while True:
                    ...
                    print("---------------Status---------------")
                    print(
                        f"Scraped {total_with_email} Emails from {i+1} URL{'s' if i > 0 else ''} and  {url_counter} URL{'s' if url_counter > 1 else ''}")
